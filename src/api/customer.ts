...
function mapCustomer(raw: any): Customer {
  return {
    ...
    dbGradeName: raw.db_grade_name ?? null,
    consultStatus: raw.consult_status,
    assignedAccountIdx: raw.assigned_account_idx ?? null,
    assignedAccountName: raw.assigned_account_name ?? null,
    distributeAt: raw.distribute_at ?? null,
...
function mapDbStats(raw: any, year: number, month: number): DbStats {
  return {
    ...
    meetingCount: raw.meeting_count ?? 0,
...
export async function getDbStats(year: number, month: number): Promise<DbStats> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const data = await customerFetch<any>(
    `${BASE_URL2}/api/app/customers/stats?year=${year}&month=${month}`,
  );
...
export async function getConsultStatuses(): Promise<ConsultStatus[]> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const data = await customerFetch<any>(`${BASE_URL2}/api/app/customers/statuses`);
...
export async function getProgressCounts(): Promise<ProgressCounts> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const data = await customerFetch<any>(`${BASE_URL2}/api/app/customers/progress`);
...
/** 상태 변경 이력 (현재 API 미제공 — 빈 배열 반환) */
export async function getStatusHistory(_limit = 20, _orderDesc = true): Promise<StatusHistoryItem[]> {
  return [];
}
